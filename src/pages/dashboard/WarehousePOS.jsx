...
// Warehouse walk-in POS. Same engine as the pharmacy POS, but every product
// carries a free-type sales unit ("carton", "box", "pack"...) that shows on the
// tile, the cart line, and the printed receipt. The price is the price for ONE
// of that unit — no conversion maths.

// Returns the unit label for a product, defaulting to "unit".
function unitOf(p) {
  const u = (p.sales_unit || '').trim()
  return u || 'unit'
}

// Pluralise a unit for display: "carton" -> "cartons". Simple + good enough.
...
export default function WarehousePOS({ brand, products, setProducts, role, perms }) {
  ...
  const visible = products.filter(p =>
    (filter === 'All' || (p.cat || p.category) === filter) &&
  ...
  // Builds an item summary line that includes the unit: "Coartem x3 cartons"
  function itemLine(i) {
    return i.name + ' x' + i.qty + ' ' + unitPlural(unitOf(i), i.qty)
  }
    ...
    if (balance > 0 && clientName !== 'Walk-in' && brand?.id) {
      try {
        await addDebt({
          ...
          description: isShortfall
            ? 'Shortfall on sale — TXN: ' + txnNo + ' | Items: ' + cart.map(itemLine).join(', ')
            : 'Credit sale — TXN: ' + txnNo + ' | Items: ' + cart.map(itemLine).join(', '),
          source: 'credit_sale',
          source_ref: txnNo,
        })
      ...
      try {
        await addDebt({
          ...
          due_date: '',
          status: 'pending',
          description: 'Sale shortfall — TXN: ' + txnNo + ' | Items: ' + cart.map(itemLine).join(', '),
          source: 'credit_sale',
          source_ref: txnNo,
        })
      } catch (e) {}
  ...
  function printReceipt(r) {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Receipt</title><style>
      *{margin:0;padding:0;box-sizing:border-box;font-family:'Courier New',monospace}
      ...
      <hr/>
      ${items.map(i => `<div style="margin-bottom:6px"><div class="b" style="font-size:12px">${i.emoji || ''} ${i.name}</div><div class="r" style="color:#666"><span>${i.qty} ${unitPlural(unitOf(i), i.qty)} x ${fmt(i.price)}</span><span>${fmt(i.price * i.qty)}</span></div></div>`).join('')}
      <hr/>
  ...
  function startScan() {
    if ('BarcodeDetector' in window) {
      ...
      navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } }).then(stream => {
        const video = document.getElementById('wpos-cam')
        ...
        <div style={{ padding: '20px' }}>
          {receipt.items.map((it, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', marginBottom: '8px' }}>
              <span>{it.emoji || '📦'} {it.name} <span style={{ color: '#aaa' }}>×{it.qty} {unitPlural(unitOf(it), it.qty)}</span></span>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', flexWrap: 'wrap' }}>
                <div>
                  ...
                  <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {items.map((it, i) => <span key={i} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '6px', background: '#f0fdfa', color: '#0f766e', fontWeight: '600' }}>{it.name} ×{it.qty} {unitPlural(unitOf(it), it.qty)}</span>)}
                  </div>
                </div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {scanning && (
            <div style={{ margin: '10px 12px', borderRadius: '12px', overflow: 'hidden', border: '2px solid #0f766e', position: 'relative', background: 'black' }}>
              <video id='wpos-cam' style={{ width: '100%', maxHeight: '180px', objectFit: 'cover', display: 'block' }} autoPlay playsInline muted />
              <button onClick={() => { setScanning(false); const v = document.getElementById('wpos-cam'); if (v?.srcObject) { v.srcObject.getTracks().forEach(t => t.stop()); v.srcObject = null } }}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: '8px', alignContent: 'start' }}>
            {visible.map(p => {
              ...
              const out = (p.cat || p.category) !== 'Services' && p.stock <= 0
              const u = unitOf(p)
              return (
              ...
              <div key={item.id} style={{ padding: '8px 12px', borderBottom: '1px solid #f9f9f9' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#bbb' }}>{fmt(item.price)} per {unitOf(item)}</div>
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  ...
                  <span style={{ fontSize: '10px', color: '#94a3b8', fontWeight: '600' }}>{unitPlural(unitOf(item), item.qty)}</span>
